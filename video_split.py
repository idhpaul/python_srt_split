...
for i in range(10):
  for j in range(10):
    for k in range(1):
        ...
        start_time = datetime.datetime.strptime(video_split_timestamp[i][j][k], '%H:%M:%S.%f')
        end_time = datetime.datetime.strptime(video_split_timestamp[i][j][k+1], '%H:%M:%S.%f')

        difference = end_time - start_time

        print(difference)
        str_delta = str(difference)

        subprocess.call(["C:\\Users\\idhpaul\\devlib\\ffmpeg\\bin\\ffmpeg.exe",
                 "-ss",
                 video_split_timestamp[i][j][k],
                 "-i",
                 "C:\\Users\\idhpaul\\Videos\\4K Video Downloader\\Elon Musk The future we're building -- and boring   TED.mp4",
                 "-t",
                 str_delta,
                 "-c",
                 "copy",
                 "output"+str(i)+"_"+str(j+1)+".mp4",
                 "-y"
                ])
